  ...
  return (
    ...
    >
      ...
      <div className="px-3 pt-2 pb-1 space-y-2">
        ...
        {/* Action Buttons: New Folder / New File / Expand All */}
        <div className="flex items-center justify-between text-[11px] text-slate-600 pt-0.5">
          <div className="flex flex-nowrap items-center gap-1 w-full">
            <button
              type="button"
              onClick={() => setNewFolderParentId(null)}
              className="flex-1 flex flex-col items-center justify-center gap-0.5 p-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 transition cursor-pointer"
              title="在根目录新建文件夹"
            >
            ...
            <button
              ...
              className="flex-1 flex flex-col items-center justify-center gap-0.5 p-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 transition cursor-pointer"
              title={`在根目录新建${currentLibrary === 'materials' ? '真题素材' : '笔记'}`}
            >
            ...
            {currentLibrary === 'materials' && (
              <button
                ...
                className="flex-1 flex flex-col items-center justify-center gap-0.5 p-1 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 transition cursor-pointer border border-emerald-100/50"
                title="批量导入真题文件夹或ZIP压缩包"
              >
